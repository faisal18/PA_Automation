...
namespace PA_Automation
{
    ...
    {
        public static void XML_foundtransction()
        {
            XmlDocument xdoc = new XmlDocument();
            xdoc.Load(@"C:\Users\faisal\Desktop\files\Testing\foundtransaction.xml");
            XmlNodeList downloaded_files = xdoc.GetElementsByTagName("File");
            foreach(XmlNode node in downloaded_files)
            {
               if (node.Attributes["FileID"].Value.Contains("8053e35b"))
                {
